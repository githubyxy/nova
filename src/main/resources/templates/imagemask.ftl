...
<div id='choose' class='section dark hide'>
    <div class='step'>What do you want to do?,
        <div id="hideLimit"></div>
        >>>>
        <div id="bb"></div>
    </div>
    <div class='sectionbody'>
        <div class='left'>
                <textarea id='message' class='message' style="height:100px;width:600px;"
                          placeholder='Type hidden text'
                          onchange="document.getElementById('bb').innerHTML=this.value.length"></textarea>
            <br/>
            <button id='encode' class='submit'>隐藏文本</button>
            <button id='decode' class='submit'>解析文本</button>
<#--            <label><input type="checkbox" id="debug"/>debug</label>-->
            <hr/>
            请选择隐藏的文件: <input type='file' id='file2'/><br/>
            <button id='encode2' class='submit'>隐藏图片</button>
            <button id='decode2' class='submit'>解析图片</button>
    ...
    var importImage = function (e) {
        ...
        reader.onload = function (event) {
            ...
            img.onload = function () {
                ...
                document.getElementById('hideLimit').innerHTML = 'the hidden text max length: ' + count + ' || the hidden file max size: ' + (mask.maxFileSize(canvas) / 1024).toFixed(2) + 'KB';
                //decode();
            };
