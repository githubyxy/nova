...
public class SimpleHttpClient {
    ...
    public String post(String url, List<NameValuePair> nameValuePairs, Map<String,String> headerMap) throws IOException {
        ...
        try {
            ...
            logger.error( "请求URL {} 时发生错误", Arrays.asList(url, e), "httpPost");
            throw e;
        } finally {
            if(response != null) {
                try {
                    ...
                    logger.error( "关闭响应流出现错误: {} ", Arrays.asList(e.getMessage(), e), "httpPost");
                }
            }
        }
        return result;
    ...
    public byte[] getByteArray(String url, Map<String,String> headerMap){
        logger.info( "请求的url: {}", Arrays.asList(url), "httpDownload");
        ...
        try {
            ...
            logger.error( "请求URL {} 时发生错误", Arrays.asList(url, e), "httpDownload");
            return null;
        } finally {
            if(response != null) {
                try {
            ...
            try {
                ...
                logger.error( "关闭输出流出现错误: {} ", Arrays.asList(e.getMessage(), e), "httpPostJson");
            }
        }
    }
    ...
    public byte[] postByteArray(String url, Object jsonObject, Map<String, String> headerMap) throws IOException {
        ...
        logger.info( "请求的url: {}, 报文体: {}", Arrays.asList(url, JSON.toJSONString(jsonObject)), "httpPostByteArray");
        ...
        try {
            ...
            int statusCode = response.getStatusLine().getStatusCode();
            if (HttpStatus.SC_OK != statusCode) {
                logger.error( "请求URL {} 时返回的statusCode为 {}", Arrays.asList(url, statusCode),
                        "httpPost");
                EntityUtils.consume(response.getEntity());
                return null;
            }
            ...
            logger.error( "请求URL {} 时发生错误", Arrays.asList(url, e), "httpPostByteArray");
            throw e;
        } finally {
            if (response != null) {
                try {
                    ...
                    logger.error( "关闭响应流出现错误: {} ", Arrays.asList(e.getMessage(), e),
                            "httpPost");
                }
            }
        }
        ...
        try {
            ...
            if(response != null) {
                try {
                    ...
                    logger.error( "关闭响应流出现错误: {} ", Arrays.asList(e.getMessage(), e), "httpDownload");
                }
            }
        }
