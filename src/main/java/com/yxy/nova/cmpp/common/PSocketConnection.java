package com.yxy.nova.cmpp.common;
...
public abstract class PSocketConnection
    ...
    /** 状态常量-尚未初始化。*/
    protected final static String NOT_INIT="Not initialized.";
    ...
    /** 状态常量－正在心跳。*/
    protected  String HEARTBEATING;

    /** 状态常量－正在接收数据。*/
    protected  String RECEIVEING;

    /** 状态常量－正在关闭连接。*/
    protected  String CLOSEING;

    /** 状态常量－连接已经正常关闭。*/
    protected  String CLOSED;

    /** 运行期错误常量-不认识的地址。(无法得到IP地址，可能DNS查询失败或IP地址格式错误)*/
    protected  String UNKNOWN_HOST;

    /** 运行期错误常量-端口号配置错误。(不是整数或范围不在0~65535内)。*/
    protected  String PORT_ERROR;

    /** 运行期错误常量-连接被拒绝。(目的主机存在，但在指定的端口上没有TCP监听)。*/
    protected  String CONNECT_REFUSE;

    /** 运行期错误常量-主机不可达。(地址正确，但路由不通或主机没开)。*/
    protected  String NO_ROUTE_TO_HOST;

    /** 运行期错误常量-接收数据超时。(接收数据时出现错误)。*/
    protected  String RECEIVE_TIMEOUT;

    /** 运行期错误常量-连接被对方关闭。*/
    protected  String CLOSE_BY_PEER;

    /** 运行期错误常量-连接被对方重置。*/
    protected  String RESET_BY_PEER;

    /** 运行期错误常量－连接已经关闭。*/
    protected  String CONNECTION_CLOSED;

    /** 错误类别常量-通信异常。*/
    protected  String COMMUNICATION_ERROR;
    ...
    /** 错误类别常量-接收数据异常。*/
    protected  String RECEIVE_ERROR;

    /** 错误类别常量-关闭连接异常。*/
    protected  String CLOSE_ERROR;
