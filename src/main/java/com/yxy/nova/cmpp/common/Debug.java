...
//---------------dump方法，用法：Debug.dump(<任意数据类型>);-------------------
   ...
   */
  public static PrintWriter out = new PrintWriter(new OutputStreamWriter(System.out, Charsets.UTF_8));
  ...
  public static void setDumpStream(OutputStream os) {
    out = new PrintWriter(new OutputStreamWriter(os, Charsets.UTF_8));
  }
  ...
  private static final void dump(String prefix,byte[]data,int offset,int length) {
    ...
    for (int i=offset; i<end; i+=16) {
      ...
      dump(indent(prefix),new String(row, Charsets.UTF_8));
    }
  ...
  public static String locate(String esc) {
    ...
    for (StringTokenizer st = new StringTokenizer(sw.toString(),"\n");
      ...
      if (str.indexOf("Exception")!=-1) { //异常描述，跳过
        ...
        continue;
      } else if (esc.equals(fullInfo)) { //一个内部使用的特殊标记
        return str;
      } else {
        int i = str.indexOf('(');
        int j = str.indexOf(')');
