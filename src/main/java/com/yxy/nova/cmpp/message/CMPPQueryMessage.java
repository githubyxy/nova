...
public class CMPPQueryMessage extends CMPPMessage
{
        ...
        throws IllegalArgumentException
    {
        ...
        } else
        {
            ...
            System.arraycopy(dateFormat.format(time).getBytes(Charsets.UTF_8), 0, super.buf, 12, dateFormat.format(time).length());
            super.buf[20] = (byte)query_Type;
            System.arraycopy(query_Code.getBytes(Charsets.UTF_8), 0, super.buf, 21, query_Code.length());
            System.arraycopy(reserve.getBytes(Charsets.UTF_8), 0, super.buf, 31, reserve.length());
