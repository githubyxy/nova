...
public class CMPPSubmitMessage extends CMPPMessage
{
            ...
            throws IllegalArgumentException
    {
        ...
        System.arraycopy(service_Id.getBytes(Charsets.UTF_8), 0, super.buf, 24, service_Id.length());
        super.buf[34] = (byte)fee_UserType;
        System.arraycopy(fee_Terminal_Id.getBytes(Charsets.UTF_8), 0, super.buf, 35, fee_Terminal_Id.length());
        super.buf[56] = (byte)tp_Pid;
        super.buf[57] = (byte)tp_Udhi;
        super.buf[58] = (byte)msg_Fmt;
        System.arraycopy(msg_Src.getBytes(Charsets.UTF_8), 0, super.buf, 59, msg_Src.length());
        System.arraycopy(fee_Type.getBytes(Charsets.UTF_8), 0, super.buf, 65, fee_Type.length());
        System.arraycopy(fee_Code.getBytes(Charsets.UTF_8), 0, super.buf, 67, fee_Code.length());
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyMMddHHmmss");
        if(valid_Time != null)
        {
            String tmpTime = String.valueOf(String.valueOf(dateFormat.format(valid_Time))).concat("032+");
            System.arraycopy(tmpTime.getBytes(Charsets.UTF_8), 0, super.buf, 73, 16);
        }
        if(at_Time != null)
        {
            String tmpTime = String.valueOf(String.valueOf(dateFormat.format(at_Time))).concat("032+");
            System.arraycopy(tmpTime.getBytes(Charsets.UTF_8), 0, super.buf, 90, 16);
        }
        System.arraycopy(src_Terminal_Id.getBytes(Charsets.UTF_8), 0, super.buf, 107, src_Terminal_Id.length());
        super.buf[128] = (byte)dest_Terminal_Id.length;
        int i = 0;
        for(i = 0; i < dest_Terminal_Id.length; i++)
            System.arraycopy(dest_Terminal_Id[i].getBytes(Charsets.UTF_8), 0, super.buf, 129 + i * 21, dest_Terminal_Id[i].length());
        ...
        System.arraycopy(reserve.getBytes(Charsets.UTF_8), 0, super.buf, loc, reserve.length());
        ...
        outStr = String.valueOf(String.valueOf((new StringBuffer(String.valueOf(String.valueOf(outStr)))).append(",msg_Content=").append(new String(msg_Content, Charsets.UTF_8))));
